export class DashboardCharts {
    ...
    renderMetrics(metrics) {
        ...
        if (this.errorRateChart) {
            ...
            this.errorRateChart.$fullTimestamps = fullTimestamps;
        ...
        if (this.responseTimeChart) {
            ...
            this.responseTimeChart.$fullTimestamps = fullTimestamps;
        ...
        if (this.throughputChart) {
            ...
            this.throughputChart.$fullTimestamps = fullTimestamps;
    ...
    updateBulk(logs) {
        ...
        for (let i = 0; i < 30; i++) {
            if (buckets[i]) {
                ...
                if (this.errorRateChart)
                    this.addPoint(this.errorRateChart, timeLabel, errorRate);
                if (this.responseTimeChart)
                    this.addPoint(this.responseTimeChart, timeLabel, avgResp);
                if (this.throughputChart)
                    this.addPoint(this.throughputChart, timeLabel, throughput);
            }
        }
        if (this.errorRateChart)
            this.errorRateChart.update('none');
    ...
    update(newLogs, allLogs) {
        ...
        if (this.errorRateChart)
            this.updateSeries(this.errorRateChart, timeLabel, errorRate);
        const withResp = newLogs.filter(l => Number(l.responseTime) > 0);
        const avgResp = withResp.length ? withResp.reduce((acc, l) => acc + Number(l.responseTime), 0) / withResp.length : 0;
        if (this.responseTimeChart)
            this.updateSeries(this.responseTimeChart, timeLabel, avgResp);
        let throughput = 0;
        if (newLogs.length > 1) {
        ...
        if (this.throughputChart)
            this.updateSeries(this.throughputChart, timeLabel, throughput);
        this.updateDistribution(allLogs);
    }
    updateDistribution(allLogs) {
        const levelTotal = {};
        allLogs.forEach(l => {
    ...
    updateSeries(chart, label, value) {
    ...
    buildTimeChartOptions() {
        return {
            ...
            plugins: {
                tooltip: {
                    callbacks: {
                        title: (items) => this.getTooltipTitle(items)
                    }
                }
            }
        };
    }
    ensureChartWidth(chart, points) {
    ...
    formatAxisTimestamp(timestamp, rangeMs) {
